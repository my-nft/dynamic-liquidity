...
async function main() {
  ...
  const YfScContract = new Contract(addresses['YF_SC'], artifacts['YF_SC'].abi, provider);
  ...
  console.log("YFSC balance token0: ", ethers.utils.formatEther(balanceToken0));
  console.log("YFSC balance token1: ", ethers.utils.formatEther(balanceToken1));
  console.log("")
  ...
  console.log(`User 1 ${address_user1} pendingReward: `, ethers.utils.formatEther(pendingReward0));
  console.log(`User 2 ${address_user2} pendingReward: `, ethers.utils.formatEther(pendingReward1));

  let originalPoolNftIds = await YfScContract.connect(signer2[0]).originalPoolNftIds(addresses[t0], addresses[t1], feeTier);
  console.log("originalPool NFT Ids of user: ", originalPoolNftIds);
